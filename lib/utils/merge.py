...
def merge_template_search(inp_list, return_search=False, return_template=False):
    ...
    seq_dict = {"feat": jt.concat([x["feat"] for x in inp_list], dim=0),
                "mask": jt.concat([x["mask"] for x in inp_list], dim=1),
                "pos": jt.concat([x["pos"] for x in inp_list], dim=0)}
    if return_search:
        x = inp_list[-1]
...
def get_qkv(inp_list):
    dict_x = inp_list[-1]
    dict_c = {"feat": jt.concat([x["feat"] for x in inp_list], dim=0),
              "mask": jt.concat([x["mask"] for x in inp_list], dim=1),
              "pos": jt.concat([x["pos"] for x in inp_list], dim=0)}
