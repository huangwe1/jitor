...
def generate_mask_cond(cfg, bs, device, gt_bbox):
    ...
    if cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == 'ALL':
        ...
        box_mask_z = jt.zeros([bs, template_feat_size, template_feat_size])
        box_mask_z[:, index, index] = 1
        box_mask_z = box_mask_z.flatten(1).bool()
        ...
        box_mask_z = jt.zeros([bs, template_size, template_size])
        box_mask_z = generate_bbox_mask(box_mask_z, gt_bbox * template_size).unsqueeze(1).float()
        box_mask_z = nn.interpolate(box_mask_z, scale_factor=1. / cfg.MODEL.BACKBONE.STRIDE, mode='bilinear',
                                    align_corners=False)
        box_mask_z = box_mask_z.flatten(1).bool()
    else:
        raise NotImplementedError
