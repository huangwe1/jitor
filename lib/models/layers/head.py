...
class Corner_Predictor(nn.Module):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        ...
        with jt.no_grad():
            ...
            self.coord_x = self.indice.repeat(self.feat_sz, 1).view(-1).float()
            self.coord_y = self.indice.repeat(1, self.feat_sz).view(-1).float()

    def execute(self, x, return_dist=False, softmax=True):
...
class CenterPredictor(nn.Module):
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        ...
        for p in self.parameters():
            if p.ndim > 1:
                jt.init.xavier_uniform_(p)

    def execute(self, x, gt_score_map=None):
    ...
    def cal_bbox(self, score_map_ctr, size_map, offset_map, return_score=False):
        score_flat = score_map_ctr.flatten(2)
        max_score = score_flat.max(dim=2, keepdims=True)
        idx = score_flat.argmax(dim=2)[0].unsqueeze(-1)  # [B, 1, 1]
        idx_y = idx // self.feat_sz
        idx_x = idx % self.feat_sz

        B = idx.shape[0]
        idx_val = idx.squeeze(-1)  # [B, 1]

        size_flat = size_map.flatten(2)  # [B, 2, H*W]
        offset_flat = offset_map.flatten(2)  # [B, 2, H*W]

        idx_for_size = idx_val.unsqueeze(1).expand([B, 2, 1])  # [B, 2, 1]
        size_list = []
        offset_list = []
        for b in range(B):
            i = int(idx_val[b, 0].item())
            size_list.append(size_flat[b, :, i:i+1])
            offset_list.append(offset_flat[b, :, i:i+1])
        size = jt.stack(size_list, dim=0)  # [B, 2, 1]
        offset = jt.stack(offset_list, dim=0).squeeze(-1)  # [B, 2]

        bbox = jt.concat([(idx_x.float().squeeze(-1) + offset[:, :1]) / self.feat_sz,
                          (idx_y.float().squeeze(-1) + offset[:, 1:]) / self.feat_sz,
    ...
    def get_pred(self, score_map_ctr, size_map, offset_map):
        ...
        idx_expanded = idx.unsqueeze(1).expand([idx.shape[0], 2, 1])
        size = jt.gather(size_map.flatten(2), 2, idx_expanded)
        offset = jt.gather(offset_map.flatten(2), 2, idx_expanded).squeeze(-1)
    ...
    def get_score_map(self, x):
        ...
        def _sigmoid(x):
            y = jt.clamp(x.sigmoid(), min_v=1e-4, max_v=1 - 1e-4)
            return y
