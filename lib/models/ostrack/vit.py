"""
Vision Transformer (ViT) in Jittor
...
from lib.models.layers.jittor_utils import Mlp, DropPath, trunc_normal_, lecun_normal_, named_apply, adapt_input_conv
...
def _init_vit_weights(module, name='', head_bias=0., jax_impl=False):
    if isinstance(module, nn.Linear):
        if name.startswith('head'):
            jt.init.zero_(module.weight)
            jt.init.constant_(module.bias, head_bias)
            ...
            if jax_impl:
                ...
                if module.bias is not None:
                    if 'mlp' in name:
                        jt.init.gauss_(module.bias, std=1e-6)
                    else:
                        jt.init.zero_(module.bias)
            else:
...
def resize_pos_embed(posemb, posemb_new, num_tokens=1, gs_new=()):
    ...
    posemb_grid = nn.interpolate(posemb_grid, size=gs_new, mode='bilinear')
