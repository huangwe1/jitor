...
class Lasot(BaseVideoDataset):
    ...
    def _read_target_visible(self, seq_path):
        ...
        with open(occlusion_file, 'r', newline='') as f:
            occlusion = jt.array(np.array([int(v) for v in list(csv.reader(f))[0]], dtype=np.uint8))
        with open(out_of_view_file, 'r') as f:
            out_of_view = jt.array(np.array([int(v) for v in list(csv.reader(f))[0]], dtype=np.uint8))

        target_visible = (1 - occlusion) * (1 - out_of_view)
    ...
    def _read_language(self, seq_path):
        try:
            ...
            with open(language_file, 'r') as file:
                language = file.readline().replace(".", "").lower()
            return language
        except:
            return None
