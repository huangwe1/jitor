...
class Got10k(BaseVideoDataset):
    ...
    def _read_meta(self, seq_path):
        ...
        try:
            ...
            language = meta_info[5].split(': ')[-1][:-1] + ", " + meta_info[6].split(': ')[-1][:-1] + ", " + \
                       meta_info[7].split(': ')[-1][:-1] + ", " + meta_info[8].split(': ')[-1][:-1]
    ...
    def _read_bb_anno(self, seq_path):
        ...
        return jt.array(np.ascontiguousarray(gt))
    ...
    def _read_target_visible(self, seq_path):
        ...
        with open(occlusion_file, 'r', newline='') as f:
            occlusion = jt.array(np.array([int(v[0]) for v in csv.reader(f)], dtype=np.uint8))
        with open(cover_file, 'r', newline='') as f:
            cover = jt.array(np.array([int(v[0]) for v in csv.reader(f)], dtype=np.uint8))

        target_visible = (1 - occlusion) * (cover > 0).uint8()

        visible_ratio = cover.float32() / 8
