...
def get_target_to_image_ratio(seq):
    anno = jt.array(seq['anno'])
    img_sz = jt.array(seq['image_size'])
...
class ImagenetVID_lmdb(BaseVideoDataset):
    ...
    def get_sequence_info(self, seq_id):
        bb_anno = jt.array(self.sequence_list[seq_id]['anno'])
        valid = (bb_anno[:, 2] > 0) & (bb_anno[:, 3] > 0)
        visible = jt.array(np.array(self.sequence_list[seq_id]['target_visible'], dtype=np.uint8)) & valid.byte()
