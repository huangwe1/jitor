...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    if mask is not None:
        mask_crop_padded = np.pad(mask_crop, ((y1_pad, y2_pad), (x1_pad, x2_pad)), mode='constant', constant_values=0)
    ...
    if output_sz is not None:
        ...
        mask_crop_padded_t = jt.array(mask_crop_padded).float32().unsqueeze(0).unsqueeze(0)
        mask_crop_padded = nn.interpolate(mask_crop_padded_t, (output_sz, output_sz),
...
def transform_image_to_crop(box_in, box_extract, resize_factor, crop_sz, normalize=False):
    box_extract_center = box_extract[0:2] + 0.5 * box_extract[2:4]
    box_in_center = box_in[0:2] + 0.5 * box_in[2:4]
    box_out_center = (crop_sz - 1) / 2 + (box_in_center - box_extract_center) * resize_factor
    box_out_wh = box_in[2:4] * resize_factor
    box_out = jt.concat((box_out_center - 0.5 * box_out_wh, box_out_wh))
    if normalize:
...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    crop_sz = jt.array([output_sz, output_sz]).float32()
