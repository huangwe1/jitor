...
class TrackingSampler:
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                ...
                template_masks = template_anno['mask'] if 'mask' in template_anno else [np.zeros((H, W), dtype=np.float32)] * self.num_template_frames
                search_masks = search_anno['mask'] if 'mask' in search_anno else [np.zeros((H, W), dtype=np.float32)] * self.num_search_frames
    ...
    def get_center_box(self, H, W, ratio=1/8):
        ...
        return jt.array([int(cx-w/2), int(cy-h/2), int(w), int(h)]).float32()
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            enough_visible_frames = visible.int32().sum().item() > 2 * (
